...
var handleProxyAction = function (req, res, method, requestUrl) {
  ...
  if (method === 'PUT' && actionUrl === 'setMockResponse') {
    ...
    writeResponse(res, 200, JSON.stringify(requestList, 0, 3));
  } else if (method === 'GET' && actionUrl === 'swagger') {

    writeResponse(res, 200, fs.readFileSync('./swagger/swagger.json', 'utf-8'));
  }
...
module.exports = {
  ...
  run: function () {
      ...
      if (!handleProxyOverrideRequest(req, res)) {
        ...
        if (mockResponses[req.url] !== undefined || mockResponses[urlBasePath] !== undefined) {
          ...
          setTimeout(function () {
            ...
            mockResponse.responseBody === Object(mockResponse.responseBody) ? res.write(JSON.stringify(mockResponse.responseBody, 0, 3)) : res.write(mockResponse.responseBody.toString(), 0, 3);
            res.end();
    ...
    serverLog("Swagger spec available at " + colors.white("http://localhost:" + port + actionUrlBase + "swagger"));

    serverLog("Public Swagger UI available at " + colors.white("http://petstore.swagger.io/index.html?url=http://localhost:" + port + actionUrlBase + "swagger"));
