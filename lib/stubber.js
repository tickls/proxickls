...
var writeResponse = function (res, statusCode, chunk) {
...
var handleProxyAction = function (req, res, method, requestUrl) {
  ...
  if (method === 'PUT' && actionUrl === 'setMockResponse') {
    ...
    argv.swaggerHost !== undefined ? swaggerHost = argv.swaggerHost : swaggerHost = "localhost";
    argv.swaggerPort !== undefined ? swaggerPort = argv.swaggerPort : swaggerPort = port;

    jsonBody.host = swaggerHost + ":" + swaggerPort;
...
module.exports = {
  ...
  run: function () {
    ...
    var server = http.createServer(function (req, res) {
      ...
      if (!handleProxyOverrideRequest(req, res)) {
        ...
        if (mockResponses[req.url] !== undefined || mockResponses[urlBasePath] !== undefined) {
          ...
          proxyLog("Returning mock data [URL: " + req.url + "] [statusCode: " + mockResponse.statusCode + "] " +
            ...
            "[Delay: " + (responseDelays[req.url] !== undefined ? responseDelays[req.url].delay : 0) + "ms]" +
          ...
          setTimeout(function () {
            ...
            if (mockResponses[req.url] !== undefined) {
              ...
              mockResponsesTimesReturnedMap[urlBasePath]++;
            }

            if (mockResponse.times !== undefined && --mockResponse.times <= 0) {
              mockResponses[req.url] = undefined;
              mockResponsesTimesReturnedMap[req.url] = undefined;
              responseDelays[req.url] = undefined;
            }

          }, responseDelays[req.url] !== undefined ? responseDelays[req.url].delay : 0);
    ...
    argv.swaggerHost !== undefined ? swaggerHost = argv.swaggerHost : swaggerHost = "http://localhost";
    argv.swaggerPort !== undefined ? swaggerPort = argv.swaggerPort : swaggerPort = port;

    serverLog("Swagger spec available at " + colors.white(swaggerHost + ":" + swaggerPort + actionUrlBase + "swagger"));

    serverLog("Public Swagger UI available at " + colors.white("http://petstore.swagger.io/index.html?url=" + swaggerHost + ":" + swaggerPort + actionUrlBase + "swagger"));
